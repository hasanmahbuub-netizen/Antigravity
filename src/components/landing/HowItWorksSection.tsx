...
export default function HowItWorksSection() {
    ...
    return (
        <section
            ref={sectionRef}
            className="relative min-h-[400vh] w-full bg-[#0A0A0A]"
        >
            {/* Sticky Container */}
                ...
                <div className="relative z-10 w-full max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
                    ...
                    <div className="space-y-12">
                        ...
                        >
                            ...
                            <h2 className="font-serif text-[42px] md:text-[56px] text-white mt-4 leading-tight">
                                Four steps to<br />
                        ...
                        <div className="space-y-4">
                            {steps.map((step, index) => (
                                <motion.button
                                    ...
                                    className={`w-full text-left p-5 rounded-2xl border transition-all duration-500 group ${activeStep === index
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        {/* Step Number */}
                                        <div
                                            className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all ${activeStep === index
                                                    ? "bg-gradient-to-br from-[#2D5F5D] to-[#1A3B3A]"
                                                    : "bg-white/5"
                                                }`}
                                        >
                                            <step.icon
                                                className={`w-5 h-5 ${activeStep === index ? "text-[#E8C49A]" : "text-white/40"
                                                    }`}
                                            />
                                        </div>
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className={`text-sm mt-1 ${activeStep === index ? "text-white/70" : "text-white/40"
                                                }`}>
                                                {step.subtitle}
                                            </p>
                                        </div>
                                    ...
                                    {activeStep === index && (
                                        <motion.p
                                            ...
                                            className="text-white/60 text-sm mt-4 pl-16 leading-relaxed"
                                        >
                                            {step.description}
                                        </motion.p>
                                    )}
                    ...
                    <div className="relative flex justify-center lg:justify-end">
                        {/* Phone Frame */}
                        <motion.div
                            className="relative w-[280px] md:w-[320px] aspect-[9/19] rounded-[40px] bg-gradient-to-b from-[#2A2A2A] to-[#1A1A1A] p-2 shadow-2xl"
                        ...
                        {/* Floating Badge */}
                        <motion.div
                            className="absolute -bottom-4 -left-4 lg:-left-8 bg-gradient-to-br from-[#2D5F5D] to-[#1A3B3A] px-6 py-3 rounded-2xl shadow-xl"
                        ...
                        >
                            <div className="flex items-center gap-2">
                                <Sparkles className="w-5 h-5 text-[#E8C49A]" />
                                <span className="text-white font-semibold">AI-Powered</span>
                            </div>
                        </motion.div>

                        {/* Glow Effect */}
                        <div className="absolute inset-0 blur-[100px] bg-[#2D5F5D]/30 rounded-full scale-150 -z-10" />
                    </div>
                </div>
            </div>
        </section>
